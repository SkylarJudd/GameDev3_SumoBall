    ...
    void applyGlitter(inout float3 finalEmission, inout float4 finalColor)
    {
        ...
        float3 finalGlitter = lerp(0, _GlitterMinBrightness, glitterAlpha) + max(pow(saturate(dot(lerp(glitterReflectionDirection, poiCam.viewDir, _GlitterBias), poiCam.viewDir)), _GlitterContrast) * _GlitterBrightness, 0);
        finalGlitter *= glitterAlpha;
        
        half3 glitterColor = _GlitterColor;
        glitterColor *= lerp(1, finalColor, _GlitterUseSurfaceColor);
        ...
        {
            finalEmission += finalGlitter * glitterColor;
        }
        else
        {
            finalColor.rgb = lerp(finalColor.rgb, finalGlitter * glitterColor, finalGlitter);
        }
        // Draw grid
